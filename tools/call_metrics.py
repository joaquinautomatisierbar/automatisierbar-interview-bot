...
def booked(call):
    """Funnel stage 4: a concrete follow-up appointment was booked. Strict — relies on
    the tightened `appointment_booked` extraction (active agreement + day + time).
    NOTE: the old `appointment_agreed` field never existed in the deployed schema, so
    that OR-clause was dead code; the real soft signal is `interview_accepted` (see
    meeting_agreed)."""
    sd = (call.get("analysis") or {}).get("structuredData") or {}
    return sd.get("appointment_booked") is True


def meeting_agreed(call):
    """Softer than booked(): the prospect actively agreed to meet/talk again, even if no
    concrete slot was pinned. Lets us separate 'agreed, slot TBD' from a steamrolled
    false-booking. Uses the existing deployed `interview_accepted` field."""
    sd = (call.get("analysis") or {}).get("structuredData") or {}
    return sd.get("interview_accepted") is True
...
def score_call(call):
    ...
    return {
        ...
        "meeting_agreed": meeting_agreed(call),
        "booked": booked(call),
    }
